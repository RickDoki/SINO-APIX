...
import com.sinosdx.service.log.dao.mapper.AnalysisGatewayLogMapper;
...
@Slf4j
@Service
public class AnalysisGatewayLogServiceImpl extends SuperServiceImpl<AnalysisGatewayLogMapper, AnalysisGatewayLog> implements
...
//        List<String> keys = Arrays.asList("1:00", "2:00", "3:00", "4:00", "5:00", "6:00", "7:00", "8:00", "9:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00", "21:00", "22:00", "23:00", "24:00")
        ...
        if (!CollectionUtils.isEmpty(appCodes)) {
            ...
            queryWrapper.in(!CollectionUtils.isEmpty(appCodes), AnalysisGatewayLog::getAppCode, appCodes);
            queryWrapper.between(AnalysisGatewayLog::getEventTime, startDate, endDate);
            List<AnalysisGatewayLog> gatewayLogs = this.baseMapper.selectList(queryWrapper);
            //  构造 首页数据统计 数据结构
            ...
            Map<String, List<AnalysisGatewayLog>> listGroup = gatewayLogs.stream().collect(Collectors.groupingBy(AnalysisGatewayLog::getGroupKey));
    ...
    public R<Object> queryGatewayLogByStatus(String appCode, Long startDate, Long endDate) {
        ...
        Long failNum = totalNum - okNum;
        AppRequestNumDTO appRequestNumDTO = new AppRequestNumDTO();
        appRequestNumDTO.setTotalNum(totalNum);
        appRequestNumDTO.setOkNum(okNum);
        appRequestNumDTO.setFailNum(failNum);
        return R.success(appRequestNumDTO);
    }
}
