<template>
  <div class="app_list_main">
    ...
    <el-dialog title="文档编辑" :visible.sync="MKDialogVisible">
      <mavon-editor
        ref="md"
        v-model="content"
        :toolbars="toolbars"
    ...
    <!-- <el-dialog title="编辑" :visible.sync="USERDialogVisible">
      <el-row>
        <el-col :span="18">
          <el-form
            ref="USERForm"
    ...
    <el-dialog title="添加成员" :visible.sync="USERDialogVisible" width="40%">
      <el-row>
        <el-col :span="18">
          <el-form
            ref="USERForm"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="userAddSure1('USERForm')"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    ...
    <div class="bottom">
      <div class="bottom_button">
        <div class="newversion" @click="pushNewVersion">新版本发布</div>
        <div class="user" @click="UserManage">成员管理</div>
        <div class="user" @click="dingyueList">订阅列表</div>
        <div class="del" @click="appDeleted">删除应用</div>
      </div>
    </div>
    <el-drawer
      title="成员管理"
    ...
    <el-drawer
      title="订阅列表"
      :before-close="handleClosedrawerList"
      :visible.sync="drawerList"
      direction="rtl"
      size="35%"
    >
    <div style="padding:10px" class="demo-drawer__content">
      <el-table
        empty-text="暂无数据"
        :header-cell-style="{ background: '#F0F2F5' }"
        border
        :data="dingyueLsit"
      >
        <el-table-column prop="creationByUsername" label="订阅人" />
        <el-table-column prop="name" label="名称" />
        <el-table-column prop="description" label="描述" />
        <el-table-column prop="isPublished" label="状态">
          <template slot-scope="scope">
...
import { Apidetail, getcurrent } from "@/api/subscribed.js";
...
export default {
  ...
  methods: {
    ...
    pushNewVersion() {
      ...
      this.$router.push({ path: "/app/newVersion?appcode=" + this.appCode });
    },
    // 选择api版本
    ...
    dingyueList() {
      ...
      getcurrent(getToken("userId_api"), this.appCode).then((res) => {
        // console.log(res);
        this.dingyueLsit = res.data
      });
    },
    // 查询现有成员
    getUserList() {
